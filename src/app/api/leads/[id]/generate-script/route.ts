...
export async function POST(
  ...
  try {
    ...
    // Determina se spostare lo stage (da analizzato → fare video)
    const analysisStages = ["HOT_LEAD", "WARM_LEAD", "COLD_LEAD"] as const;
    const shouldMoveStage = (analysisStages as readonly string[]).includes(lead.pipelineStage);
    ...
    const updatedLead = await db.lead.update({
      ...
      data: {
        ...
        ...(shouldMoveStage && { pipelineStage: "FARE_VIDEO" }),
      },
    });
